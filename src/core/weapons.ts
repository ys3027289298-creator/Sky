...
export class TargetingSystem {
  ...
  update(candidates: { id: string; visible: boolean; inRange: boolean }[], dt: number, switchTarget = false) {
    const valid = candidates.filter(c => c.visible && c.inRange);
    if (!this.lock.targetId) {
      const next = valid[0];
      if (next) this.lock = { targetId: next.id, progress: 20, lostTimer: 0 };
    } else if (switchTarget) {
      const next = valid.find(c => c.id !== this.lock.targetId);
      if (next) this.lock = { targetId: next.id, progress: 20, lostTimer: 0 };
    }
    ...
    else { this.lock.lostTimer += dt; this.lock.progress = Math.max(0, this.lock.progress - 35 * dt); if (this.lock.lostTimer > 1.4) this.lock = { targetId: null, progress: 0, lostTimer: 0 }; }
    return this.lock;
  }
}
